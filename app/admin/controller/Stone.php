...
//        bind('Stone', $this);
    ...
    protected function _throwMixed($data)
    {
        ...
        if (false !== $data) {                  //执行成功
            if (!Request::isAjax()) {           //不是ajax
                ...
                $this->success();               //执行到此，不会继续执行
            }
        } else {                                //执行失败
            $this->error();                     //执行到此，不会继续执行
        }
        return $data;
    }
    ...
    protected function _actionBeforeAfter($action = 'index')
    {
        ...
        //TODO::执行子类中的前置方法级操作(注：需要返回数据)
        if (method_exists($this, $_beforeAction)) {
            $data = $this->$_beforeAction($data);
        }

        //TODO::执行从模型中获取数据(注：需要返回数据）
        $data = self::model()::$_linkActionAttr($data);

        //TODO::执行子类的中后置方法级操作(注：需要返回数据）
