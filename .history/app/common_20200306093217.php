...
if (!function_exists('array_intersect_mixed')) {
     ...
     * @return array
     */
    function array_mix(array $master=[], string $master_key ='id', array $branch=[], string $branch_key ='id', bool $force = false): array
    {
        ...
        if ($branch) {
            ...
            foreach ($master as $k => $v) {
                $temp = 
                if (isset($branch_arr[$v[$master_key]]) || $force) {
                    $master_arr[$k] =  $v;                       //返回交集
                    if (isset($branch_arr[$v[$master_key]])) {                                //合并数组
                        $master_arr[$k]['merge'] = $branch_arr[$v[$master_key]];
                    }
                }
            }
        } else {
...
function array_key(array $data,$column,$col=''){
    $group = array();
    foreach($data as $key =>$v){
        if($col) {
            $group[$v[$column]] = $v[$col];
        }else{
            $group[$v[$column]] = $v;
        }
    }
