...
class Menu extends Model
{
    ...
    public function menuGet(int $role_id = null, int $aid = null, bool $ALL = false, bool $force = false): array
    {
        ...
        $where3['role_id'] =  $role_id;
                $where2['aid'] = $aid;
               
                $access = Db::name('admin_auth')->whereOr([$where3,$where2])->fetchSql()->select()->toArray();
        //权限过滤(如是是超级管理员返回所有权限)
        if($role_id != 1){
    ...
    public function roleAccess($role_id = null, $aid = null,$force = false)
    {
        ...
        if ($role_id == 1) {
            ...
            if (!$access) {
                ...
                $access = Db::name('admin_auth')->whereOr([$where,$where2])->fetchSql()->select()->toArray();
                $datas[$label] = $access;
