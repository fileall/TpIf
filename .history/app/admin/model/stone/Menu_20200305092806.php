    ...
    public function menuGet(int $role_id = null, int $aid = null, bool $ALL = false, int $paren_pid = null): array
    {
        ...
        if (!$menu) {
            ...
            if (is_numeric($paren_pid)) {
                $this->where('parent_id', $paren_pid);
            }
            $menu = $this->order(['parent_id', 'list_order'])
                    ->select()
                    ->toArray();
            if ($paren_pid == null) {
                $data_menu['all'] = $menu;
...
//               $t = array_intersect_mix($menu,'access',$access);
    ...
    public function roleAccess($role_id = 2,$aid = null)
    {
        $access = [];
        if ($role_id == 1) {
            $access = 'all';
        // if ($force) $access = list_to_tree($access, "id", "parent_id");
            ...
            if (!$access) {
                $where = [
                    ['role_id','=',$role_id],
                    ['aid','=',$aid],'OR'];
                $access = Db::name('admin_auth')->where($where)->select()->toArray();dump($access);die;
