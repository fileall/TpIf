...
class Menu extends Model
{
    ...
    public function menuGet(int $role_id = null, int $aid = null,bool $force = false,bool $ALL = false): array
    {
        ...
        if (!$menu) {
            ...
            if ($role_id != 1) {
                ...
                if ($access || ) {
                    $menu = array_mix($menu, 'id', $access, 'menu_id', $force);
                } else {
                    $menu = [];
                }
            }
    ...
    public function roleAccess($role_id = null, $aid = null)
    {
        ...
        $aid != null && $where2['aid'] = $aid;
        $access = Db::name('admin_auth')->where($where);
        if($aid != null) $access->whereOr($where2);
        $list = $access->select()->toArray();
    ...
    public function subMenuGet(int $role_id = null, int $aid = null, int $_menuId = null)
    {
        ...
        if (!$sub_menu) {
            ...
            if ($role_id != 1) {
                $access = self::roleAccess($role_id, $aid);
                if ($access) {
                    $sub_menu = array_mix($sub_menu, 'id', $access, 'menu_id');
                } else {
                    $sub_menu = [];
                }
            } 
            $data_sub_menu[$_menuId]  = $sub_menu;
            debug_get() or cache($file, $data_sub_menu);
        }
