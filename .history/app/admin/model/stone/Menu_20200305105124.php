...
class Menu extends Model
{
     ...
     * @return array
     */
    public function menuGet(int $role_id = null, int $aid = null, bool $ALL = false, int $force = false): array
    {
        $menu = [];
        ...
        if ($paren_pid == null) {
            $menu = isset($data_menu['all']) ? $data_menu['all'] : [];
        } else {
            $menu = isset($data_menu[$paren_pid]) ? $data_menu[$paren_pid] : [];
        }
        ...
        if (!$menu) {
            ...
            $menu = $model::where($where)->order(['parent_id', 'list_order'])
                    ->select()
                    ->toArray();
            if ($paren_pid == null) {
                $data_menu['all'] = $menu;
        ...
        $access = self::roleAccess(2, $aid);
        if ($access == 'all') {
            return $menu;
        } elseif ($access) {
            $t = array_intersect_mix($menu, 'id', $access,'menu_id',force);dump($t);die;
        } else {
            return [];
        }
