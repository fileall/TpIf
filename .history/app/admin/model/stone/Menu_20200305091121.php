...
class Menu extends Model
{
     ...
     * @return array
     */
    public function menuGet(int $role_id = null,int $aid = nu bool $ALL = false, int $paren_pid = null): array
    {
        if ($ALL) {
...
//               $t = array_intersect_mix($menu,'access',$access);
    ...
    public function roleAccess($role_id = null, $force = false)
    {
        ...
        // if ($force) $access = list_to_tree($access, "id", "parent_id");
            ...
            if (!$access) {
                $access = Db::name('admin_auth')->where('role_id', $role_id)->where()->value('menu_id,');
