    ...
    protected function _throwMixed($data)
    {
        if (false !== $data) {                  //执行成功
            if (!Request::isAjax()) {           //不是ajax
                $this->assign($data['assign']);dump(233);die;
                return  view();                //执行到此，不会继续执行
            } else {     dump(333);die;                       //ajax
                $this->success($this->_msg,$this->_url,$this->_data,$this->_wait); //执行到此，不会继续执行
            }
        } else {                                //执行失败
            $this->error($this->_msg,$this->_url,$this->_data,$this->_wait);   //执行到此，不会继续执行
        }
        return $data;
    }
