...
class Loam extends Stone
{
    ...
    // ];
    protected $db = Db::class;   //Db操作类;
    protected $view = View::class;   //Db操作类;
    protected $request = request::class;   //Db操作类;
    ...
    {
        $this->_menuId = Request::get('menu_id', '0');dump($this->_menuId);die;
        //同级菜单
        if ($this->_menuId) {
            $_sub_menu = face('menu')::subMenuGet(1,1, $this->_menuId);
            $assign = [
                '_sub_menu' => $_sub_menu ?? [],
