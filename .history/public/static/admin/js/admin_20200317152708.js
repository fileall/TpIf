...
layui.define(["element", "jquery"], function (exports) {
    ...
    admin = new function () {
        ...
        this.init = function (url) {
            ...
            admin.initBgColor();
            admin.initDevice();console.log(url);
            $.getJSON(url, function (data, status) {  console.log(url);console.log(data);
                if (data == null) {
                    admin.msg_error('暂无菜单信息');
                } else {
                    admin.initHome(data.homeInfo);
        ...
        this.initHome = function (data) {
            sessionStorage.setItem('adminHomeHref', data.href);
            $('#adminHomeTabId').html('<i class="' + data.icon + '"></i> <span>' + data.title + '</span>');
            $('#adminHomeTabId').attr('admin-id', data.href);
            $('#adminHomeTabIframe').html('<iframe width="100%" height="100%" frameborder="0"  src="' + data.href + '"></iframe>');
        };
        ...
        this.initTab = function () {
            ...
            if (urlArr.length >= 2) {
                ...
                if (!checkTab) {
                    ...
                    var adminHomeTab = $('#layuiminiHomeTab').attr('lay-id'),
                    adminHomeHref = sessionStorage.getItem('layuiminiHomeHref');
        ...
        this.checkTab = function (tabId, isIframe) {
            ...
            // 判断sessionStorage是否有
            var adminTabInfo = JSON.parse(sessionStorage.getItem("layuiminiTabInfo"));
    ...
    $('body').on('click', '[data-iframe-tab]', function () {
        ...
        if (!checkTab) {
            var adminInfo = JSON.parse(sessionStorage.getItem("adminTabInfo"));
            if (adminTabInfo == null) {
                adminTabInfo = {};
            }
            adminTabInfo[tabId] = {href: href, title: title}
            sessionStorage.setItem("adminTabInfo", JSON.stringify(layuiminiTabInfo));
